...
use crate::infra::validation::{SecurityValidator, CheckStatus};
...
impl SecurityValidateArgs {
    ...
    pub async fn execute(&self, ctx: &ExecutionContext) -> Result<()> {
        ...
        info!("Validating security in namespace: {}", namespace);

        // Create Kubernetes client
        let client = self.create_client(&namespace).await?;

        // Run security validation
        let validator = SecurityValidator::new(client);
    ...
    async fn output_results(
        ...
        if ctx.json_output {
            ...
            let output = if report.healthy {
                CommandOutput::success_with_data("Security validation passed", serde_json::to_value(&report)?)
            } else {
                CommandOutput::failure_with_data("Security validation failed", serde_json::to_value(&report)?)
            };
            ...
            if results.healthy {
                ...
                print_error("Security validation failed - review warnings and errors");
...
mod tests {
    ...
    #[test]
    fn test_security_validate_args() {
        assert!(true);
    }
}
