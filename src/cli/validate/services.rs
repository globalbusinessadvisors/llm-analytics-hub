...
use crate::infra::validation::{ServiceValidator, CheckStatus};
...
impl ServiceValidateArgs {
    ...
    pub async fn execute(&self, ctx: &ExecutionContext) -> Result<()> {
        ...
        info!("Validating services in namespace: {}", namespace);

        // Create Kubernetes client
        let client = self.create_client(&namespace).await?;

        // Run service validation
        let validator = ServiceValidator::new(client);
    ...
    async fn output_results(
        ...
        if ctx.json_output {
            ...
            let output = if report.healthy {
                CommandOutput::success_with_data("Service validation passed", serde_json::to_value(&report)?)
            } else {
                CommandOutput::failure_with_data("Service validation failed", serde_json::to_value(&report)?)
            };
            ...
            if results.healthy {
                ...
                anyhow::bail!("Service validation failed");
            }
        }
